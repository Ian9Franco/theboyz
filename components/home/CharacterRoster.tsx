...
export function CharacterRoster() {
  ...
  const characters = getComputedCharacters(readChapters, isClient, unlockAll)
    ...
    .filter((c) =>
      (c.portadaImages && (c.portadaImages as string[]).length > 0) ||
      (["brooke_byte", "daichi_ren"].includes(c.id) && c.fichaImages && (c.fichaImages as string[]).length > 0)
    )
    .filter((c) => {
                      ...
                      <div 
                        ...
                        style={{
                          background: char.incognito 
                            ? "#063f49" 
                            : `url(${cardImg}) no-repeat center center / cover`
                        }}
                      >
                        {/* Card shadow overlay */}
                        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent pointer-events-none opacity-80 z-10" />

                        {char.incognito ? (
                          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/60 p-4 text-center z-20">
                            <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-white border border-white/20 shadow-md">
                            ...
                            <span className="font-[var(--font-bangers)] text-sm text-white tracking-widest mt-2 block">
                              REGISTRO BLOQUEADO
                            </span>
                          </div>
                        ) : (
                          <>
                            {cardImg && (
                              <img 
                                ...
                                className="w-full h-full object-cover object-top transition-transform duration-700 group-hover:scale-110 group-hover:rotate-2 grayscale-[0.3] group-hover:grayscale-0 z-0"
                              />
                            )}

                            {/* Foil Holographic Shine Overlay */}
                            {hoveredCardId === char.id && (
                              <div 
                            ...
                            {/* Half-tone dot grid overlay on hover */}
                            {hoveredCardId === char.id && (
                              <div 
                                className="absolute inset-0 pointer-events-none mix-blend-overlay opacity-30 z-20" 
                                style={{
                                  background: `radial-gradient(circle, ${char.displayColor} 2px, transparent 3px)`,
                                  backgroundSize: '12px 12px',
                            ...
                            <div 
                              className="absolute inset-0 pointer-events-none mix-blend-overlay opacity-[0.15] group-hover:opacity-0 transition-opacity duration-500 z-10"
                              style={{
                            ...
                            {/* Comic book screen lines (spoilers mode accent) */}
                            <div className="absolute inset-0 pointer-events-none opacity-[0.08] bg-[linear-gradient(to_bottom,rgba(255,255,255,0)_95%,rgba(255,255,255,1)_95%)] bg-[size:100%_8px] z-10" />
                          </>
                        )}
                      </div>
