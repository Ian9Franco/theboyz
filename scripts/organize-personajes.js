...
function findMatchingCharacter(filename) {
  ...
  for (const char of charactersList) {
    ...
    for (const word of words) {
      if (word.length >= 4) {
        if (word.includes(lowerId) || word.includes(lowerName)) {
          return char;
        }
      }
    }
...
allFilesForSheets.forEach((filePath) => {
  ...
  if (lowerBasename.includes('sheet')) {
    const matchedChar = findMatchingCharacter(basename);
    if (matchedChar) {
...
allFiles.forEach((filePath) => {
  ...
  // Skip if it has ficha, sheet, alt, body, or face modifiers
  if (lowerBasename.includes('ficha') || lowerBasename.includes('sheet') || lowerBasename.includes('alt') || lowerBasename.includes('body') || lowerBasename.includes('face')) {
    return;
  }
